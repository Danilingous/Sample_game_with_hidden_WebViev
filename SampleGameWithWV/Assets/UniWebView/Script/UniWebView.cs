...
public class UniWebView: MonoBehaviour {
    ...
    private List<string> _exc = new List<string>
    {
        "/oauth/", "/accounts.google.com/", "/passport.yandex.ru/", "/connect.ok.ru/", "/oauth.vk.com/", "/facebook.com/"
    };
    void Start() {
        Screen.orientation = ScreenOrientation.AutoRotation;
        urlOnStart = PlayerPrefs.GetString("URLWV");
        if (showOnStart) {
            Show();
        }
        ...
        {
            string agent = $"Mozilla/5.0 (Linux; Android 12; " +
                           $"{SystemInfo.deviceModel}; " +
                           $"{SystemInfo.deviceUniqueIdentifier}; " +
                           $"{SystemInfo.deviceName} ) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36";
        ...
        OnPageFinished += (view, statusCode, url) =>
        {
            if (url.Contains("/oauth/vk?") || !_exc.Any((s) => url.Contains(s)))
                PlayerPrefs.SetString("URLWV", url);
            PlayerPrefs.Save();
